...
http_conn::HTTP_CODE http_conn::parse_request_line(char *text)
{
    ...
    *m_url++ = '\0';
    ...
    if (strcasecmp(method, "GET") == 0)
        m_method = GET; //报文的请求方法，本项目只用到GET和POST
...
一部分是语法错误导致的BAD_REQUEST，一部分是do_request的返回结果.
*/
http_conn::HTTP_CODE http_conn::process_read()
{
    //初始化从状态机状态、HTTP请求解析结果
    LINE_STATUS line_status = LINE_OK;
    HTTP_CODE ret = NO_REQUEST; //NO_REQUEST：请求不完整，需要继续读取请求报文数据
    char *text = 0;

    //这里为什么要写两个判断条件？第一个判断条件为什么这样写？
    //具体的在主状态机逻辑中会讲解。

    //parse_line为从状态机的具体实现  m_check_state：主状态机的状态
    while ((m_check_state == CHECK_STATE_CONTENT && line_status == LINE_OK) || ((line_status = parse_line()) == LINE_OK))
    {
        ...
        {   //解析请求行
            ...
            if (ret == BAD_REQUEST)  //BAD_REQUEST：HTTP请求报文有语法错误
                return BAD_REQUEST;
            break;
        }
        ...
        {   //解析请求头
            ...
            else if (ret == GET_REQUEST) //GET_REQUEST：获得了完整的HTTP请求
            {
                return do_request();
            }
            break;
...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    //如果请求资源为/5，表示跳转到picture.html，即图片请求页面
    else if (*(p + 1) == '5')
    {
        char *m_url_real = (char *)malloc(sizeof(char) * 200);
    ...
    //如果请求资源为/6，表示跳转到video.html，即视频请求页面
    else if (*(p + 1) == '6')
    {
        char *m_url_real = (char *)malloc(sizeof(char) * 200);
    ...
    //如果请求资源为/7，表示跳转到fans.html，即关注页面
    else if (*(p + 1) == '7')
    {
        char *m_url_real = (char *)malloc(sizeof(char) * 200);
...
//connection记录连接状态，用于告诉浏览器端保持长连接。 通知浏览器端是保持连接还是关闭
...
void http_conn::process()
{
    ...
    {
        //注册并监听读事件  EPOLLIN：表示对应的文件描述符可以读（包括对端SOCKET正常关闭）
        modfd(m_epollfd, m_sockfd, EPOLLIN, m_TRIGMode);
        return;
    }
    ...
    //注册并监听写事件  EPOLLOUT：表示对应的文件描述符可以写
    modfd(m_epollfd, m_sockfd, EPOLLOUT, m_TRIGMode);
}
